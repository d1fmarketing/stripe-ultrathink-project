...
// src/handlers/stripeSubmitEvidence.ts
var stripeSubmitEvidence_exports = {};
__export(stripeSubmitEvidence_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(stripeSubmitEvidence_exports);
...
async function handler(evt) {
  ...
  if (merchant?.access_token) {
    stripe = new stripe_esm_node_default(merchant.access_token, { apiVersion: "2025-07-30.basil" });
  } else if (merchant?.stripe_account_id) {
    stripe = new stripe_esm_node_default(process.env.STRIPE_SECRET, { apiVersion: "2025-07-30.basil" });
